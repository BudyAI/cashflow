...
export function parseAgingExcel(
  ...
  for (let r = 0; r < Math.min(rows.length, 20); r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      if (mapping.contactName && cell === mapping.contactName)
        idx.contactName = c;
      if (mapping.issueDate && cell === mapping.issueDate) idx.issueDate = c;
      if (mapping.invoiceNumber && cell === mapping.invoiceNumber)
        idx.invoiceNumber = c;
    }
  ...
  if (rowsProcessed === 0) {
    throw new Error(
      "No valid rows with a due date and amount were found in this file.",
    );
  }
  if (sawMixedCurrency) {
    throw new Error(
      "This file mixes USD and ILS in the currency column. Mixed-currency aging totals are not supported. " +
        "Upload separate reports per currency or use a single currency for all rows.",
    );
  }
  if (skippedNoDueDate > 0)
    errors.push(`${skippedNoDueDate} row(s) skipped — no parseable due date.`);
  if (skippedNoAmount > 0)
    errors.push(`${skippedNoAmount} row(s) skipped — zero or missing amount.`);
