...
export function parseExcel(buffer: Buffer): ParseResult {
  ...
  if (content.trimStart().startsWith('<') || content.includes('<html') || content.includes('<table')) {
    ...
    // Direct HTML table file (e.g. sheet001.htm)
    const rows = extractHtmlTable(content)
    if (rows.length < 2) {
      return { transactions: [], errors: ['HTML file appears to have no table data'] }
    }
  ...
  if (!sheetName) {
    return { transactions: [], errors: ['No sheets found in the file'] }
  }
  ...
  if (rows.length < 2) {
    return {
      transactions: [],
      errors: [`File appears to be empty (found ${rows.length} non-empty rows in sheet "${sheetName}")`],
    }
  }
