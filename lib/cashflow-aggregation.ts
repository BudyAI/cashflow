...
export function aggregateCashflowFromTransactions(
  transactions: TxRow[],
): CashflowSummary {
  ...
  const incomeCategoryMap = new Map<
    string,
    { name: string; color: string; totals: Record<string, number> }
  >();
  ...
  for (const tx of transactions) {
    ...
    if (tx.amount > 0) {
      ...
      const incomeEntry = incomeCategoryMap.get(key)!;
      incomeEntry.totals[month] = (incomeEntry.totals[month] ?? 0) + tx.amount;
    } else {
      ...
      const expenseEntry = expenseCategoryMap.get(key)!;
      expenseEntry.totals[month] =
        (expenseEntry.totals[month] ?? 0) + Math.abs(tx.amount);
    }
  }
  ...
  const trueStart =
    firstTx?.balance != null ? firstTx.balance - firstTx.amount : 0;
